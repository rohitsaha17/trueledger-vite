...
export default function AdminMedia() {
  ...
  async function load() {
    const data = await api.get<MediaItem[]>("/media/all");
    setItems(data);
  }
  ...
  async function save() {
    ...
    if (editing.id) {
      await api.put(`/media/${editing.id}`, editing);
    } else {
      await api.post("/media", editing);
    }
    setSaving(false);
    setEditing(null);
    setSearchParams({});
  ...
  async function togglePublish(item: MediaItem) {
    await api.put(`/media/${item.id}`, { published: !item.published });
    load();
  }
